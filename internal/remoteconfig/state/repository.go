...
func (r *Repository) Update(update Update) ([]string, error) {
	...
	if r.tufVerificationEnabled {
		...
		if err != nil {
			return nil, fmt.Errorf("tuf roots clone error: %v", err)
		}
		err = tmpRootClient.updateRoots(update.TUFRoots)
		if err != nil {
			return nil, fmt.Errorf("tuf update roots error: %v", err)
		}

		updatedTargets, err = tmpRootClient.validateTargets(update.TUFTargets)
		if err != nil {
			return nil, fmt.Errorf("tuf roots validate targets error: %v", err)
		}
	} else {
		updatedTargets, err = unsafeUnmarshalTargets(update.TUFTargets)
		if err != nil {
			return nil, fmt.Errorf("tuf unmarshal targets error: %v", err)
		}
	}
	...
	for _, path := range update.ClientConfigs {
		...
		if err != nil {
			return nil, fmt.Errorf("config path parsing error: %v", err)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("config metadata instantiation error: %v", err)
		}
		config, err := parseConfig(parsedPath.Product, raw, m)
		if err != nil {
			return nil, fmt.Errorf("product parsing error of %s: %v", parsedPath.Product, err)
		}
	...
	if r.tufVerificationEnabled {
		...
		if err != nil {
			return nil, fmt.Errorf("root version extraction error: %v", err)
		}
